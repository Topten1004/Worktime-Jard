...
namespace Worktime.Global
{
    public static class TokenJWT
    {
        ...
        public static JwtSecurityToken creerTokenJWT(string login, string mdp, WorktimeDbContext db, TOKEN_EXPIRATION tokenExp = TOKEN_EXPIRATION.MOIS)
        {
            var user = db.Users.Where(u => u.Login == login && u.Mdp == mdp).FirstOrDefault();
            if (user != null)
            {
